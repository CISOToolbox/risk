...
(function () {
    ...
    window.catalogCreate = function () {
        ...
        if (init) {
            ...
            Object.assign(D, JSON.parse(JSON.stringify(init)));
        }
    ...
    window.catalogOpen = function (id) {
        ...
        _get(id, function (record) {
            ...
            localStorage.setItem("ebios_catalog_active", id);
            try {
                var _parsed = JSON.parse(record.data);
                Object.keys(D).forEach(function (k) { delete D[k]; });
                Object.assign(D, _parsed);
                if (typeof ensureKeys === "function")
                    ensureKeys();
                if (typeof _initDataAndRender === "function") {
    ...
    window.catalogDuplicate = function (id) {
        _get(id, function (record) {
            ...
            try {
                ...
                if (d.context)
                    d.context.societe = newRecord.name;
    ...
    window.catalogExportAll = function () {
        _getAll(async function (items) {
            ...
            URL.revokeObjectURL(a.href);
            if (typeof showStatus === "function")
                showStatus(t("catalog.exported_all"));
        });
    };
        ...
        _getAll(function (items) {
            ...
            if (filtered.length === 0) {
                ...
                for (var i = 0; i < filtered.length; i++) {
                    ...
                    if (!isActive)
                        h += '<button class="catalog-btn-sm catalog-btn-del" data-click="catalogDelete" data-args=\'' + _da(item.id) + '\' data-stop data-tip="' + esc(t("catalog.delete")) + '">✕</button>';
                    h += '</div>';
                    h += '</div>';
                }
            }
