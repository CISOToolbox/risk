...
(function () {
    ...
    function _mesuresContexte() {
        var couverture = {};
        (D.sop_detail || []).forEach(function (d) {
            var etiquette = (String(d.sop || "") + "/" + _attackLabel(d.phase || "")).replace(/^\/|\/$/g, "");
            String(d.mesure_proposee || "").split(",").forEach(function (morceau) {
                var mid = morceau.trim().split(" - ")[0].trim();
                if (mid)
                    (couverture[mid] = couverture[mid] || []).push(etiquette);
            });
        });
        ...
        return toutes.slice(0, MAX_MESURES_CONTEXTE).map(function (m) {
        ...
        socle: function () {
            ...
            return {
                user: "Context: " + JSON.stringify({ societe: D.context.societe, socle: D.context.socle, reglementation: D.context.reglementation }) +
                    ...
                    "\n\nBaseline controls with gaps (not fully conformant, with a documented écart): " + JSON.stringify(ecarts.map(function (e) {
    ...
    function _assainirActions(liste) {
        ...
        liste.forEach(function (s) {
            ...
            if (ACTIONS_MESURE.indexOf(action) >= 0) {
                s.action = action;
                return;
            }
            delete s.action;
    ...
    function _suggestionsDe(type, result) {
        var suggestions = _normalizeSuggestions(type, result);
        if (PANNEAUX_MESURES.indexOf(type) >= 0)
    ...
    function _ajoutDetails(ancien, ajout) {
    ...
    function _sopMesureCelluleHTML(ph) {
        if (ph && ph.mesure_existante_id) {
            ...
            if (m) {
                ...
                if (ajout || titreChange) {
                    ...
                    if (ajout)
                        c += '<br><span style="color:var(--ct-low)">+ ' + esc(ajout) + '</span>';
                    return c;
                }
    ...
    function _fusionnerDetails(ancien, ajout) {
        var a = String(ancien || "").trim();
        var b = String(ajout || "").trim();
        if (!a)
            return b;
     ...
     */
    function _apercuEnrichissementHTML(s) {
     ...
     */
    function _reutiliserMesure(s, rattacher) {
        if (!s || s.action !== "enrich" || !s.id)
            return "";
        var cible = D.measures.find(function (m) { return m.id === s.id; });
        if (!cible)
        ...
        if (s.details)
            cible.details = _fusionnerDetails(cible.details || "", s.details);
                ...
                if (p.mesure_existante_id) {
                    ...
                    if (deja) {
                        ...
                        if (p.mesure_ajustement) {
                            ...
                            deja.details = _fusionnerDetails(deja.details || "", String(p.mesure_ajustement).slice(0, 5000));
                        }
    ...
    window._aiRunSOP = async function (ssId, mode) {
        ...
        try {
            ...
            if (!promptObj)
                throw new Error("Strategic scenario " + ssId + " not found");
            if (userText) {
    ...
    function _lierRefEco(ppId, mid, libelle) {
        ...
        try {
            ...
            if (result.v_resid) {
                ...
                if (result.justification)
                    h += '<div class="ai-card-details" style="margin-top:8px">' + esc(result.justification) + '</div>';
                h += '</div>';
            }
            p.body.innerHTML = h;
            // Store for accept
