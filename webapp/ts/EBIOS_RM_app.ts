...
function sel(section: string, idx: number, field: string, val: unknown, options: (string | number)[], labelFn?: (o: string | number) => string) {
    let h = `<select data-s="${section}" data-i="${idx}" data-f="${field}" data-change="_updateFieldFromEl" data-pass-el>`;
    h += `<option value=""></option>`;
    for (const o of options) h += `<option value="${o}" ${String(val) === String(o) ? 'selected' : ''}>${labelFn ? esc(String(labelFn(o))) : o}</option>`;
    h += `</select>`;
    return h;
}
...
function renderIndicators() {
    ...
    document.getElementById("indicators")!.innerHTML = counts.map(
        ([d,n]) => `<div class="ct-kpi" data-density="dense"><div class="ct-kpi-tone"></div><div class="ct-kpi-body"><div class="ct-kpi-label">${esc(d)}</div><div class="ct-kpi-value">${n}</div></div></div>`
    ).join("");
    // Badges counts
...
function renderSynthesis() {
    ...
    distH += '</div>';
    if (nonEval > 0) distH += `<p class="ct-muted ct-text-meta ct-mt-2">${t("ebios.misc.ss_not_evaluated", {n: nonEval})}</p>`;
    ...
    D.ss.forEach((s, i) => {
        ...
        if (ri && rr) {
            if (rr < ri) evol = `<span class="ct-text-low ct-strong">&#x2198; ${t("ebios.synth.ameliore")}</span>`;
            else if (rr === ri) evol = `<span class="text-muted">= ${t("ebios.synth.identique")}</span>`;
            else evol = `<span class="ct-text-critical ct-strong">&#x2197; ${t("ebios.synth.degrade")}</span>`;
    ...
    const statutColor: Record<string, string> = {"Terminé":"var(--ct-low)","En cours":"var(--ct-high)","À étudier":"var(--ct-critical)"};
