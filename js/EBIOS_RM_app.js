...
function _refOnToggle(uid, section, idx, field, ids, el, single) {
    ...
    if (section.startsWith("comp_")) {
        ...
        if (D.socle_complementaires[fwId]) {
            if (!D.socle_complementaires[fwId][refKey])
                D.socle_complementaires[fwId][refKey] = { conformite: "", ecart: "", mesures_prevues: "" };
...
function renderIndicators() {
    ...
    document.getElementById("indicators").innerHTML = counts.map(([d, n]) => `<div class="indicator"><div class="number">${n}</div><div class="desc">${d}</div></div>`).join("");
    // Badges counts
...
function _toggleReferentielNow(fwId) {
    ...
    if (pos >= 0) {
        ...
        if (meta && !D.socle_complementaires[fwId]) {
            D.socle_complementaires[fwId] = Object.fromEntries(meta.measures.map(m => [m.ref, { conformite: "", ecart: "", mesures_prevues: "" }]));
        }
    }
    renderSocleRefs();
    renderContext();
...
function updateRefField(fwId, idx, field, value, cast) {
    ...
    if (typeof window._persistSettings === "function")
        window._persistSettings();
    else
        _autoSave();
}
...
function moveSOPPhase(idx, dir) {
    ...
    if (target === groupStart && dir === -1)
        return;
    if (idx === groupStart)
        return; // la première ligne ne bouge pas
...
function renderSynthesis() {
    ...
    D.ss.forEach((s, i) => {
        ...
        if (risk === t("ebios.risk.eleve"))
            eleve++;
        else if (risk === t("ebios.risk.moyen"))
            moyen++;
        else if (risk === t("ebios.risk.faible"))
            faible++;
        else
            nonEval++;
    });
    ...
    distH += '</div>';
    if (nonEval > 0)
        distH += `<p style="color:var(--text-muted);font-size:0.85em;margin-top:8px">${t("ebios.misc.ss_not_evaluated", { n: nonEval })}</p>`;
    ...
    D.ss.forEach((s, i) => {
        ...
        if (ri && rr) {
            if (rr < ri)
                evol = `<span style="color:var(--green);font-weight:600">&#x2198; ${t("ebios.synth.ameliore")}</span>`;
            else if (rr === ri)
                evol = `<span class="text-muted">= ${t("ebios.synth.identique")}</span>`;
            else
                evol = `<span style="color:var(--red);font-weight:600">&#x2197; ${t("ebios.synth.degrade")}</span>`;
        }
        else
...
function renderAll() {
    try {
        ...
        if (_tr)
            _tr.innerHTML = _getSettingsButtonHTML() + _getGithubLinkHTML("https://github.com/CISOToolbox/risk");
...
function _cv(cell) {
    ...
    if (typeof v === "object" && v !== null) {
        if (v.result !== undefined)
            return v.result; // formule avec résultat caché
        if (v.richText)
...
function ensureKeys() {
    ...
    if (typeof D.socle_complementaires !== "object" || Array.isArray(D.socle_complementaires))
        D.socle_complementaires = {};
    for (const fwId of D.referentiels_actifs) {
        const meta = REFERENTIELS_META[fwId];
        if (!meta)
            continue;
        ...
        if (!existing) {
            ...
            for (const m of meta.measures) {
                if (!(m.ref in existing))
                    existing[m.ref] = { conformite: "", ecart: "", mesures_prevues: "" };
            }
        }
    }
...
try {
    ...
    if (tr)
        tr.innerHTML = _getSettingsButtonHTML() + _getGithubLinkHTML("https://github.com/CISOToolbox/risk");
