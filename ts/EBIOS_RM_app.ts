...
let REFERENTIELS_META: Record<string, EbiosRefMeta> = Object.fromEntries(
    Object.entries(_REFERENTIELS_CATALOG).map(([k, v]) => [k, {...v}])
) as Record<string, EbiosRefMeta>;
...
function _ensureTemplate(cb: () => void) {
...
function _updateFieldFromEl(el: HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement) {
...
function _refSliderChange(fwId: string, idx: number, val: string) {
    updateRefField(fwId, idx, "conformite", parseInt(val), "number");
}

function _setContextField(key: string, val: string) {
    D.context[key] = val;
    renderContext();
    _persist("context");
}

function _setGravityField(idx: number, field: string, rerender: boolean, val: string) {
...
var _RISK_CANONICAL: Record<string, string> = {"Élevé":"Élevé","Elevé":"Élevé","Eleve":"Élevé","High":"Élevé","Moyen":"Moyen","Medium":"Moyen","Faible":"Faible","Low":"Faible"};

function _toCanonicalRisk(val: string): string { return _RISK_CANONICAL[val] || val; }
function _displayRisk(val: string): string {
...
function _setRiskMatrix(ri: number, vi: number, val: string) {
...
function _effBadgeClick(el: HTMLElement) {
    const s = el.nextElementSibling as HTMLElement;
    s.style.display = "inline";
    el.style.display = "none";
    const sl = s.querySelector("select") as HTMLSelectElement | null;
    if (sl) { sl.focus(); sl.click(); try { sl.showPicker(); } catch (e: any) {} }
}

function _newSRFor(idx: number) {
    const id = newSR();
    if (id) updateSROVRef(idx, "sr_id", id);
}

function _newOVFor(idx: number) {
    const id = newOV();
...
function computeExposition(menace: number | null): string {
...
function gravLabel(n: EbNum): string {
    const gs = D.gravity_scale.find(g => String(g.niveau) === String(n));
    return gs ? gs.label : "";
}
function riskLevel(gNum: EbNum, v: EbNum | undefined): string {
    if (!gNum || !v) return "";
    for (const row of D.risk_matrix) {
        try { if (parseInt(String(row.g)) === parseInt(String(gNum))) return _displayRisk(row.levels[parseInt(String(v)) - 1] || ""); } catch (e: any) {}
    }
    return "";
}
function socleStatut(conf: EbNum | null): string {
...
function soclePriorite(conf: EbNum | null): string {
    if (conf === "" || conf === null) return "";
...
function computeSSGravity(erList: string): EbNum {
    if (!erList) return "";
    let max = 0;
...
// ═══════════════════════════════════════════════════════════════════════
function _range(a: EbNum, b: EbNum) { const r: number[] = []; for (let i = Math.max(1,a||1); i <= Math.min(4,b||4); i++) r.push(i); return r; }
function inp(section: string, idx: number, field: string, val: unknown, type = "text", cls = "") {
    ...
    if (type === "date" && val != null && val !== "" && !/^\d{4}-\d{2}-\d{2}$/.test(String(val))) type = "text";
...
function sel(section: string, idx: number, field: string, val: unknown, options: (string | number)[]) {
...
function ta(section: string, idx: number, field: string, val: string) {
...
function ta_ref(fwId: string, idx: number, field: string, val: string) {
    const rows = Math.max(2, Math.ceil((val || "").length / 40));
...
function delBtn(section: string, idx: number) {
...
function _attackResolveId(value: string): string {
...
function _attackLabel(value: string): string {
    const id = _attackResolveId(value);
    if (id) return t("ebios.attack." + id);
    return String(value || "").replace(/^\d+\.\s*/, "").trim();
}
function _sopPhaseSelect(idx: number, value: string): string {
...
function toggleDICT(section: string, idx: number, field: string, dim: string, el: HTMLElement) {
    const current: string[] = (D[section][idx][field] || "").split(",").map((s: string) => s.trim()).filter(Boolean);
...
let _refCounter = 0;
function refSelect(section: string, idx: number, field: string, val: string, options: CtRefOption[], single?: boolean) {
...
function _refOnToggle(uid: string, section: string, idx: number, field: string, ids: string[], el: HTMLInputElement, single: boolean) {
    _saveState();
...
function _refOnRemove(section: string, idx: number, field: string, removeId: string) {
    _saveState();
    if (section.startsWith("comp_")) {
        const fwId = section.slice(5);
        const refKey = _compRefKey(fwId, idx);
        if (D.socle_complementaires[fwId] && D.socle_complementaires[fwId][refKey]) {
            const current = String(D.socle_complementaires[fwId][refKey][field] || "");
            const parts = current.split(",").map((s: string) => s.trim()).filter((s: string) => !s.startsWith(removeId));
...
function _refOnFlush(section: string, field: string) {
...
function _reRenderForField(section: string, field: string) {
    const renders: Record<string, (() => void) | (() => void)[]> = {
...
async function renderHistory() {
    await _renderSnapshotsPanel({
        ...
        orgField: "societe",
        keys: {
            create: "ebios.history.create",
            encrypt: "ebios.history.encrypt",
            decrypt: "ebios.history.decrypt",
            encryption_active: "ebios.history.encryption_active",
            none: "ebios.history.none",
            col_name: "ebios.history.col_name",
            col_date: "ebios.history.col_date",
            col_org: "ebios.history.col_societe",
            col_actions: "ebios.history.col_actions",
            restore: "ebios.history.restore",
            export: "ebios.history.export",
            hint: "ebios.history.hint",
        },
    });
}
...
var _PANEL_RENDER: Record<string, () => void> = {
...
function renderIndicators() {
    ...
    for (const [k,v] of [["vm",D.vm],["bs",D.bs],["pp",D.pp],["er",D.er],["ss",D.ss],
        ...
        { const el = document.getElementById("count-"+k); if(el) el.textContent = String(v.length); }
    const socle = D.socle_type === "iso" ? D.socle_iso : D.socle_anssi;
    const el = document.getElementById("count-socle"); if(el) el.textContent = String(socle.length);
...
const _gravCache: Record<number, { scale: EbiosGravityLevel[]; matrix: EbiosRiskMatrixRow[] }> = {};
...
function setGravityLevels(n: number) {
    ...
    if (_gravCache[n]) {
        ...
        const defaults: Record<number, EbiosGravityLevel[]> = {
            3: [
        ...
        const matrices: Record<number, EbiosRiskMatrixRow[]> = {
            3: [
...
function setSocleType(type: "anssi" | "iso") {
    _saveState();
    D.socle_type = type;
...
function _toggleReferentielNow(fwId: string) {
    _saveState();
...
function _compRefKey(fwId: string, idx: number | string): string {
    const meta = REFERENTIELS_META[fwId];
...
function updateRefField(fwId: string, idx: number | string, field: string, value: string | number, cast?: string) {
    if (!D.socle_complementaires[fwId]) return;
    const refKey = _compRefKey(fwId, idx);
    if (!D.socle_complementaires[fwId][refKey]) D.socle_complementaires[fwId][refKey] = {conformite: "", ecart: "", mesures_prevues: ""};
    D.socle_complementaires[fwId][refKey][field] = cast === "number" ? (value === "" ? "" : parseInt(String(value))) : value;
    if (typeof window._persistSettings === "function") window._persistSettings();
    else _autoSave();
}
...
function importVendorPP(event: Event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            if (data.pp_export) {
                ...
                ppList = data.vendors.map(function(v: any) {
                    var ex = v.exposure || {};
                    ...
                    return {
                        ...
                        measures: (v.measures || []).map(function(m: any) {
            ...
            var clamp = function(v: number) { var n = Math.round(v); return n >= 1 ? Math.min(n, 4) : ""; };
            ppList.forEach(function(v: any) {
                ...
                var existantes: string[] = [], complementaires: string[] = [];
                var statutMap: Record<string, string> = {"termine":"Terminé","en_cours":"En cours","planifie":"Planifié","a_lancer":"À lancer"};
                vMeasures.forEach(function(m: any) {
...
function addSocleMeasure(socleIdx: number) {
    _saveState();
...
function _srNom(id: string) { const s = (D.sr_list||[]).find(x => x.id === id); return s ? s.nom : ""; }
function _ovNom(id: string) { const o = (D.ov_list||[]).find(x => x.id === id); return o ? o.nom : ""; }
function _srFull(id: string) { return id ? id + " - " + _srNom(id) : ""; }
function _ovFull(id: string) { return id ? id + " - " + _ovNom(id) : ""; }
...
function updateSROVRef(idx: number, field: "sr_id" | "ov_id", val: string) {
    _saveState();
...
function srSelectWidget(idx: number, val: string) {
...
function ovSelectWidget(idx: number, val: string) {
...
function renderSROV() {
    ...
    D.srov.forEach((s, i) => {
        ...
        var _prioColors: Record<string, string> = {};
...
function renderER() {
    ...
    const cats: [string, string][] = [["financier", t("ebios.gravity.col_impact_financier")], ["reputation", t("ebios.gravity.col_impact_reputation")], ["reglementaire", t("ebios.gravity.col_impact_reglementaire")], ["donnees_perso", t("ebios.gravity.col_impact_donnees_perso")], ["operationnel", t("ebios.gravity.col_impact_operationnel")]];
...
function _updateErGraviteCat(el: HTMLSelectElement) {
    const i = parseInt(el.getAttribute("data-i")!);
...
function _toggleErGraviteCat() {
    const cb = document.getElementById("er-grav-cat-toggle") as HTMLInputElement | null;
    D.context = D.context || {};
...
function _buildEcoSVG(ppList: EbEcoPoint[], title: string) {
    const W = 1000, H = 900, CX = W/2, CY = 390, R = 270, M = 15;
    const maxMenace = 5;

    function menaceToR(m: number) { return R * (1 - Math.min(m, maxMenace) / maxMenace); }
    function degXY(deg: number, r: number): [number, number] {
        const rad = (deg - 90) * Math.PI / 180;
        return [CX + r * Math.cos(rad), CY + r * Math.sin(rad)];
    }

    const catMap: Record<string, EbEcoPoint[]> = {};
    ...
    const fiabs: [string, string, number][] = [["#dc2626",t("ebios.eco.fiab_faible"),170],["#f59e0b",t("ebios.eco.fiab_moyenne"),250],["#eab308",t("ebios.eco.fiab_bonne"),340],["#16a34a",t("ebios.eco.fiab_elevee"),420]];
...
function addEcoMeasure(ecoIdx: number) {
    _saveState();
...
function _syncSopMeasuresToResiduals(sopDetailIdx: number, oldVal: string, newVal: string) {
...
function _setSOPPhase(idx: number, value: string) {
    _saveState();
    if (value === "__other__") {
...
function _setSOPPhaseText(idx: number, value: string) {
...
function _findSOPStart(sopId: string): number {
...
function moveSOPPhase(idx: number, dir: number) {
    _saveState();
...
function delSOPPhase(idx: number) {
    _saveState();
    const mySop = _findSOPGroup(idx);
    const groupStart = _findSOPStart(mySop);
    if (idx === groupStart) {
...
function _computeSOPVop() {
    ...
    const sopVop: Record<string, number> = {};
    const sopTaux: Record<string, number> = {};
...
function _ssVInit() {
    ...
    const sopToSS = _sopToSS();
    const vInitMap: Record<string, number> = {};
    for (const [sopId, ssSet] of Object.entries(sopToSS)) {
        for (const ssId of ssSet) {
            const v = sopVop[sopId] || 0;
...
function renderSynthesis() {
    ...
    const ssVinit: Record<string, number> = {};
    for (const [sopId, ssSet] of Object.entries(sopToSS)) {
        const v = sopVop[sopId] || 0;
        for (const ssId of ssSet) {
    ...
    function buildMatrix(target: string, getV: (sp: EbSynthPos) => number) {
        var grid: Record<string, CtMatrixItem[]> = {};
    ...
    const origColor: Record<string, string> = {"Socle":"#dcfce7","Écosystème":"#fef9c3","SOP":"#fed7aa","Complémentaire":"#dbeafe"};
    const statutColor: Record<string, string> = {"Terminé":"var(--green)","En cours":"var(--orange)","À étudier":"var(--red)"};
...
function _loadExcelJS() {
    return _loadScript("https://cdn.jsdelivr.net/npm/exceljs@4.4.0/dist/exceljs.min.js", { integrity: "sha384-Pqp51FUN2/qzfxZxBCtF0stpc9ONI6MYZpVqmo8m20SoaQCzf+arZvACkLkirlPz", crossOrigin: "anonymous", onStart: () => showStatus(t("ebios.status.loading_exceljs")), onDone: () => showStatus(t("ebios.status.exceljs_loaded")) });
}
...
function _readExcelData(wb: ExcelJS.Workbook) {
    ...
    D.sr_list = [];
    D.ov_list = [];
    const srSeen: Record<string, string> = {}, ovSeen: Record<string, string> = {};
    const wsSROV = wb.getWorksheet("2-Couples SR-OV");
    if (wsSROV) {
        for (let i = 0; i < 30; i++) {
            const r = 5 + i;
...
function ensureKeys() {
    ...
    D.gravity_scale.forEach(g => {
        ...
        if (!("label" in g)) (g as any).label = "";
        if (!("description" in g)) (g as any).description = "";
    ...
    for (const fwId of D.referentiels_actifs) {
        ...
        if (!existing) {
            ...
            const migrated: Record<string, EbiosCompEntry> = {};
    ...
    if (D.risk_matrix.length === 0) {
        ...
        if (D.gravity_scale.length === 0) {
            function _g(niv: number, label: string, desc: string): EbiosGravityLevel {
                return {niveau:niv, label:label, description:desc,
                    impact_financier:"", impact_reputation:"", impact_reglementaire:"",
                    impact_donnees_perso:"", impact_operationnel:""};
            }
        ...
        var _matrices: Record<number, EbiosRiskMatrixRow[]> = {
...
function _synthesisData() {
    ...
    const positions: EbSynthPos[] = [], rows: EbSynthRow[] = [];
    ...
    socleRows.forEach(function(s) {
        const c = s.conformite;
        if (c !== "" && c !== null && !isNaN(c)) {
            totalConf += parseFloat(String(c)); socle.count++;
...
function _riskHex(level: string): string {
...
async function exportSynthesisPPTX() {
    try {
        ...
        function matrixSlide(title: string, getV: (sp: EbSynthPos) => number, intro: string, reading: string) {
        ...
        function tableSlide(title: string, head: string[], rows: any[][], intro: string, maxRows: number) {
            const FS = 10, tableW = W - 1, per = Math.max(1, maxRows || 15);
            rows = rows || [];
            const pages: any[][][] = [];
            for (let i = 0; i < rows.length; i += per) pages.push(rows.slice(i, i + per));
            if (!pages.length) pages.push([]);
            const headRow = head.map(function(h: string) { return { text: h, options: { bold: true, color: "FFFFFF", fill: { color: ACCENT } } }; });
            let firstSlide: any = null;
            pages.forEach(function(pg: any[][], idx: number) {
                ...
                const body = [headRow as any[]].concat(pg.map(function(r: any[]) { return r.map(function(c: any) { return (c && typeof c === "object") ? c : String(c == null ? "" : c); }); }));
        ...
        const sev = function(lvl: string) { var c = _riskColorName(lvl); return c === "red" ? 3 : c === "orange" ? 2 : c === "green" ? 1 : 0; };
        const distOf = function(key: "riskInit" | "riskResid") { var d = { eleve: 0, moyen: 0, faible: 0 }; sd.rows.forEach(function(r) { var c = _riskColorName(r[key]); if (c === "red") d.eleve++; else if (c === "orange") d.moyen++; else if (c === "green") d.faible++; }); return d; };
        const dInit = distOf("riskInit"), dResid = distOf("riskResid");
        const riskCell = function(lvl: string) { return { text: lvl || "—", options: { fill: { color: _riskHex(lvl) }, color: "1F2937", bold: true, align: "center" } }; };
        ...
        [{ n: sd.socle.nonApp, l: t("ebios.misc.non_applique_label"), c: "FCA5A5" }, { n: sd.socle.partiel, l: t("ebios.misc.partiel_label"), c: "FED7AA" }, { n: sd.socle.applique, l: t("ebios.misc.applique_label"), c: "DCFCE7" }].forEach(function(k: any, i: number) {
            const x = 1.4 + i * 3.6;
        ...
        const distRow = function(d: { eleve: number; moyen: number; faible: number }, y: number, label: string) {
            ...
            [{ n: d.eleve, l: t("ebios.misc.eleve_label"), c: "FCA5A5" }, { n: d.moyen, l: t("ebios.misc.moyen_label"), c: "FED7AA" }, { n: d.faible, l: t("ebios.misc.faible_label"), c: "DCFCE7" }].forEach(function(k: any, i: number) {
                var x = 2.7 + i * 2.9;
...
function _svgToPng(svg: string, targetW: number, scale?: number): Promise<EbReportImg> {
...
function _riskMatrixSVG(getV: (sp: EbSynthPos) => number): string | null {
    ...
    const sd = _synthesisData(); const grid: Record<string, CtMatrixItem[]> = {};
...
function _riskMatrixRefSVG() {
    ...
    let maxYLen = 0; gLabels.forEach((l: string) => { if (String(l).length > maxYLen) maxYLen = String(l).length; });
...
async function _reportImages() {
    const imgs: Record<string, EbReportImg> = {};
    try {
        if (typeof _buildEcoSVG === "function" && (D.pp || []).length) {
            ...
            const resid = D.pp.map(p => { const eco = (D.eco || []).find(e => (e.pp_id || "").split(" - ")[0].trim() === p.id) || {} as Partial<EbiosEco>;
            ...
            const _eco = (s: string) => s.replace('viewBox="0 0 1000 900"', 'viewBox="-140 90 1280 650"').replace(/font-size="9"/g, 'font-size="12"');
...
function _resolveRedacteur() {
    ...
    let saved = "";
    try { saved = localStorage.getItem("ct_report_redacteur") || ""; } catch (e: any) {}
    const name = ((typeof window.prompt === "function" ? window.prompt(t("ebios.report.ask_redacteur"), saved) : "") || "").trim();
    if (name) { try { localStorage.setItem("ct_report_redacteur", name); } catch (e: any) {} return name; }
    return saved;
}
...
function _reportData() {
    const sd = _synthesisData();
    const c = D.context || {};
    const srName: Record<string, string> = {}; (D.sr_list || []).forEach(x => srName[x.id] = x.nom);
    const ovName: Record<string, string> = {}; (D.ov_list || []).forEach(x => ovName[x.id] = x.nom);
    const isAnssi = D.socle_type !== "iso";
    const socleRows = (isAnssi ? D.socle_anssi : D.socle_iso) || [];
    ...
    const bucket = (lvl: string) => { const cn = _riskColorName(lvl); return cn === "red" ? "eleve" : cn === "orange" ? "moyen" : cn === "green" ? "faible" : null; };
    ...
    const sopSS: Record<string, Set<string>> = {}, stepsBySop: Record<string, { phase: string; action: string; controle: string; efficacite: string }[]> = {};
    ...
    const _conf = (v: EbNum | null | undefined) => (v === "" || v == null) ? "" : S(v) + " %";
    return {
...
function _injectImages(zip: PizZip, imgs: Record<string, EbReportImg>) {
    const EMU = 9525;
...
async function exportWordReport() {
    try {
        ...
        catch (imgErr: any) { console.warn("Cartographies non intégrées:", imgErr);
